...
import { Shield, Clock, DollarSign, ArrowRight, Calendar, Zap, CreditCard, BarChart3, Database, type LucideIcon } from 'lucide-react';
import { getTranslations } from 'next-intl/server';
import { buildProblems, buildFeaturesData, buildMetrics, buildFAQs, buildPhases, buildProfiles, buildRelatedServices, buildDashboardViews, type FeatureItemData } from '@/i18n/builders/service-pages';
...
export default async function CompleteBillingPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations('services/complete-billing');
  ...
  const dashboardViews = buildDashboardViews(t.raw('dashboardShowcase.views'));
    ...
    <>
      ...
      <section className="py-12 md:py-16 space-y-12">
        <DashboardShowcase
          ...
          subtitle={t('dashboardShowcase.subtitle')}
