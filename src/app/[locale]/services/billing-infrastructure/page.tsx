...
import { Clock, DollarSign, CreditCard, BarChart3, Shield, Database, Zap, Calendar, ArrowRight, TrendingUp, type LucideIcon } from 'lucide-react';
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return generatePageMetadata({
    ...
    namespace: 'billingInfrastructure',
...
export default async function BillingInfrastructurePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const t = await getTranslations('services/billing-infrastructure');
    ...
    <>
      ...
      <section className="py-12 md:py-16 space-y-12">
        <div className="rounded-xl border bg-card p-8 md:p-12">
          <h2 className="text-3xl font-bold tracking-tight mb-6">{t('metrics.title')}</h2>
          <p className="text-muted-foreground mb-8">{t('metrics.subtitle')}</p>
